...
`;
  ...
  async loadSettings() {
    ...
    const preAccumulatedChronologyPerspectiveKeys = perspectiveKeys
      .filter((key) => !ACCUMULATED_CHRONOLOGY_PERSPECTIVE_KEYS.includes(key));
    ...
    const preSemioticLinguisticPerspectiveKeys = perspectiveKeys
      .filter((key) => !SEMIOTIC_LINGUISTIC_PERSPECTIVE_KEYS.includes(key) && !ACCUMULATED_CHRONOLOGY_PERSPECTIVE_KEYS.includes(key));
